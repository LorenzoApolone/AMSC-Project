...
public:
  ...
  /// @brief Callable shorthand: f(x) delegates to value(x)
  double operator()(const vector<double> &x) const { return value(x); }
...
/**
 * @struct Particle
 * @brief Represents a single particle in the swarm.
 */
struct Particle {
    std::vector<double> position;
    std::vector<double> velocity;
    std::vector<double> p_best_pos;
    double p_best_val;
    double current_val;
    Particle(int dim);
};
