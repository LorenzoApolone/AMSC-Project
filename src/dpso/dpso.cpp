...
void regroup_particles(std::vector<Particle>& local_swarm, int dim, int rank, int size) {
    ...
    for (int i = 0; i < local_n; ++i) {
        ...
        int off    = 0;
        std::copy(recv_buffer.begin() + base + off, recv_buffer.begin() + base + off + dim,
                  local_swarm[i].position.begin());      off += dim;
        std::copy(recv_buffer.begin() + base + off, recv_buffer.begin() + base + off + dim,
                  local_swarm[i].velocity.begin());      off += dim;
        std::copy(recv_buffer.begin() + base + off, recv_buffer.begin() + base + off + dim,
                  local_swarm[i].best_position.begin()); off += dim;
...
OutputObject dpso(const TestFunction& f,
    ...
    while (true) {
        ...
        for (int s = 0; s < num_sub_swarms; ++s) {
            ...
            process_sub_swarm(swarm, start, end, dim, lb, ub, v_max,
                              w, c1, c2, f, rank, iter, max_iter);
        }
        if (remainder > 0) {
        ...
        struct { double val; int rank; } local_min, global_min;
        ...
        MPI_Allreduce(&local_avg, &global_avg_dist, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
        ...
        int stop_flag = 0;
        if (rank == 0)
            stop_flag = stop_manager.should_stop(global_best_val, global_avg_dist) ? 1 : 0;
        MPI_Bcast(&stop_flag, 1, MPI_INT, 0, MPI_COMM_WORLD);
        if (stop_flag) break;
    }

    struct { double val; int rank; } loc, glob;
