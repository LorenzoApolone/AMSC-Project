#include "create_network.hpp"
#include <algorithm>
#include <random>
#include <set>
#include <utility>
#include <vector>

namespace {

std::vector<int> collect_rewire_candidates(
    int N, int u, int v, const std::set<std::pair<int, int>> &edges,
    const std::vector<std::pair<int, int>> &to_add) {
  std::vector<int> candidates;
  candidates.reserve(N);

  for (int candidate = 0; candidate < N; ++candidate) {
    if (candidate == u || candidate == v) {
      continue;
    }

    auto edge = std::make_pair(std::min(u, candidate), std::max(u, candidate));
    if (edges.count(edge) > 0 ||
        std::find(to_add.begin(), to_add.end(), edge) != to_add.end()) {
      continue;
    }

    candidates.push_back(candidate);
  }

  return candidates;
}

} // namespace


void create_network(int N, double p,
                    std::vector<std::vector<int>> &adjacency_list) {
  std::random_device rd;
  std::mt19937 gen(rd());
  create_network(N, p, adjacency_list, gen);
}

void create_network(int N, double p,
                    std::vector<std::vector<int>> &adjacency_list,
                    unsigned int seed) {
  std::mt19937 gen(seed);
  create_network(N, p, adjacency_list, gen);
}

void create_network(int N, double p,
                    std::vector<std::vector<int>> &adjacency_list,
                    std::mt19937 &gen) {
  adjacency_list.assign(N, std::vector<int>());
  if (N <= 0) {
    return;
  }

  std::set<std::pair<int, int>> edges;
  for (int i = 0; i < N; ++i) {
    int r1 = (i + 1) % N;
    int r2 = (i + 2) % N;
    if (i != r1) {
      edges.insert({std::min(i, r1), std::max(i, r1)});
    }
    if (i != r2) {
      edges.insert({std::min(i, r2), std::max(i, r2)});
    }
  }

  std::uniform_real_distribution<double> dis(0.0, 1.0);
  std::vector<std::pair<int, int>> to_remove;
  std::vector<std::pair<int, int>> to_add;

  for (const auto &e : edges) {
    if (dis(gen) >= p) {
      continue;
    }

    int u = e.first;
    int v = e.second;
    std::vector<int> candidates =
        collect_rewire_candidates(N, u, v, edges, to_add);

    // Small swarms can have no legal rewiring target; keep the edge as-is.
    if (candidates.empty()) {
      continue;
    }

    std::uniform_int_distribution<int> node_dis(
        0, static_cast<int>(candidates.size()) - 1);
    int new_v = candidates[node_dis(gen)];

    to_remove.push_back(e);
    to_add.push_back({std::min(u, new_v), std::max(u, new_v)});
  }

  for (const auto &e : to_remove) {
    edges.erase(e);
  }
  for (const auto &e : to_add) {
    edges.insert(e);
  }

  for (const auto &e : edges) {
    adjacency_list[e.first].push_back(e.second);
    adjacency_list[e.second].push_back(e.first);
  }
}

void create_scale_free_network(int N, int m,
                               std::vector<std::vector<int>> &adjacency_list) {
  std::random_device rd;
  std::mt19937 gen(rd());
  create_scale_free_network(N, m, adjacency_list, gen);
}

void create_scale_free_network(int N, int m,
                               std::vector<std::vector<int>> &adjacency_list,
                               unsigned int seed) {
  std::mt19937 gen(seed);
  create_scale_free_network(N, m, adjacency_list, gen);
}

void create_scale_free_network(int N, int m,
                               std::vector<std::vector<int>> &adjacency_list,
                               std::mt19937 &gen) {
  adjacency_list.assign(N, {});
  if (N <= 0) {
    return;
  }

  if (m < 1) {
    m = 1;
  }
  if (m >= N) {
    m = N - 1;
  }
  if (m <= 0) {
    return;
  }

  int m0 = m + 1;
  if (m0 > N) {
    m0 = N;
  }

  std::vector<int> degree(N, 0);
  std::vector<int> pool;

  for (int i = 0; i < m0; ++i) {
    for (int j = i + 1; j < m0; ++j) {
      adjacency_list[i].push_back(j);
      adjacency_list[j].push_back(i);
      degree[i]++;
      degree[j]++;
    }
  }

  for (int i = 0; i < m0; ++i) {
    for (int k = 0; k < degree[i]; ++k) {
      pool.push_back(i);
    }
  }

  for (int new_node = m0; new_node < N; ++new_node) {
    std::vector<int> targets;
    targets.reserve(m);

    if (pool.empty()) {
      for (int t = 0; t < new_node && static_cast<int>(targets.size()) < m; ++t) {
        targets.push_back(t);
      }
    } else {
      std::uniform_int_distribution<int> pick(
          0, static_cast<int>(pool.size()) - 1);
      while (static_cast<int>(targets.size()) < m) {
        int cand = pool[pick(gen)];
        if (cand == new_node) {
          continue;
        }
        if (std::find(targets.begin(), targets.end(), cand) != targets.end()) {
          continue;
        }
        targets.push_back(cand);
      }
    }

    for (int t : targets) {
      adjacency_list[new_node].push_back(t);
      adjacency_list[t].push_back(new_node);
      degree[new_node]++;
      degree[t]++;
      pool.push_back(t);
    }

    for (int k = 0; k < degree[new_node]; ++k) {
      pool.push_back(new_node);
    }
  }
}

void create_random_network(int N, double p,
                           std::vector<std::vector<int>> &adjacency_list) {
  std::random_device rd;
  std::mt19937 gen(rd());
  create_random_network(N, p, adjacency_list, gen);
}

void create_random_network(int N, double p,
                           std::vector<std::vector<int>> &adjacency_list,
                           unsigned int seed) {
  std::mt19937 gen(seed);
  create_random_network(N, p, adjacency_list, gen);
}

void create_random_network(int N, double p,
                           std::vector<std::vector<int>> &adjacency_list,
                           std::mt19937 &gen) {
  adjacency_list.assign(N, {});
  if (N <= 0 || p < 0.0 || p > 1.0) {
    return;
  }

  std::uniform_real_distribution<double> dis(0.0, 1.0);
  for (int i = 0; i < N; ++i) {
    for (int j = i + 1; j < N; ++j) {
      if (dis(gen) < p) {
        adjacency_list[i].push_back(j);
        adjacency_list[j].push_back(i);
      }
    }
  }
}
