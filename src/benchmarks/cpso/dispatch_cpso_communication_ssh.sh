...
usage() {
  printf '%s\n' \
    "Usage: dispatch_cpso_communication_ssh.sh --host HOST --remote-src REMOTE_SRC [options]" \
    "" \
    "Options:" \
    ...
    "  --tag NAME                 Custom run tag (default: cpso_comm_greedy_ablation)." \
    ...
    "" \
    "This launcher always runs the CPSO communication ablation battery."
}
...
CMD=(bash "$SCRIPT_DIR/dispatch_cpso_ssh.sh" --host "$HOST" --remote-src "$REMOTE_SRC" --battery communication --tag "$TAG")
if [[ -n "$SEEDS_ARG" ]]; then
  CMD+=(--seeds "$SEEDS_ARG")
fi
