...
usage() {
  printf '%s\n' \
    "Usage: dispatch_cpso_validation_k28_ssh.sh --host HOST --remote-src REMOTE_SRC [options]" \
    "" \
    "Options:" \
    ...
    "  --tag NAME                 Custom run tag (default: cpso_validation_k28)." \
    ...
    "" \
    "This launcher always runs the CPSO validation battery for the k=28 sanity check."
}
...
CMD=(bash "$SCRIPT_DIR/dispatch_cpso_ssh.sh" --host "$HOST" --remote-src "$REMOTE_SRC" --battery validation --tag "$TAG")
if [[ -n "$SEEDS_ARG" ]]; then
  CMD+=(--seeds "$SEEDS_ARG")
fi
