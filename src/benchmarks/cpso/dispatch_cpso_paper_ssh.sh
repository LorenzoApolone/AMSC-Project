...
usage() {
  printf '%s\n' \
    "Usage: dispatch_cpso_paper_ssh.sh --host HOST --remote-src REMOTE_SRC [options]" \
    "" \
    "Options:" \
    ...
    "  --tag NAME                 Custom run tag (default: cpso_paper_clean)." \
    ...
    "" \
    "This launcher always runs the main clean CPSO paper battery (battery=all)."
}
...
CMD=(bash "$SCRIPT_DIR/dispatch_cpso_ssh.sh" --host "$HOST" --remote-src "$REMOTE_SRC" --battery all --tag "$TAG")
if [[ -n "$SEEDS_ARG" ]]; then
  CMD+=(--seeds "$SEEDS_ARG")
fi
