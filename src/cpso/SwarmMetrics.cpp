...
void compute_avg_distance(const std::vector<SubSwarm>& swarms, 
                          ...
                          double *mpi_allreduce_time_s) {
  double local_sum_dist = 0.0;
  int local_particle_count = 0;

  // Recompute the metric at every iteration
  ...
  if (use_mpi) {
#if CPSO_HAVE_MPI
    const auto time_allreduce = [&](auto *sendbuf, auto *recvbuf,
                                    MPI_Datatype datatype) {
      const double start = MPI_Wtime();
      MPI_Allreduce(sendbuf, recvbuf, 1, datatype, MPI_SUM, MPI_COMM_WORLD);
      if (mpi_allreduce_time_s != nullptr) {
        *mpi_allreduce_time_s += MPI_Wtime() - start;
      }
    };

    time_allreduce(&local_sum_dist, &global_sum_dist, MPI_DOUBLE);
    time_allreduce(&local_particle_count, &global_particle_count, MPI_INT);
#endif
  } else {
  ...
  last_avg_distance = (global_particle_count > 0)
                          ? (global_sum_dist / global_particle_count)
                          : 0.0;
}
