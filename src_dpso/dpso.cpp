...
Particle::Particle(int dim) {
    ...
    current_val = std::numeric_limits<double>::max();
}

double random_double(double min, double max, int rank) {
    thread_local std::mt19937 gen(rank * 10000 + omp_get_thread_num() * 100 + 12345); 
...
void apply_harmony_search(std::vector<Particle>& swarm, 
    ...
    for (int d = 0; d < dim; ++d) {
        ...
        if (random_double(0.0, 1.0, rank) < HMCR) {
            if (sub_swarm_size > 0) {
                ...
                new_harmony[d] = swarm[random_member_idx].p_best_pos[d];
    ...
    for (int i = start_idx; i < end_idx; ++i) {
        double d = euclidean_dist(new_harmony, swarm[i].p_best_pos);
        if (d < min_dist) {
            min_dist = d;
            nearest_idx = i;
        }
    }
    if (nearest_idx != -1 && new_harmony_val < swarm[nearest_idx].p_best_val) {
        swarm[nearest_idx].p_best_pos = new_harmony;
        swarm[nearest_idx].p_best_val = new_harmony_val;
    }
}
...
void regroup_particles(std::vector<Particle>& local_swarm, int dim, int rank, int size) {
    ...
    for (int i = 0; i < local_n; ++i) {
        ...
        std::copy(recv_buffer.begin() + base + offset, 
                  recv_buffer.begin() + base + offset + dim, 
                  local_swarm[i].p_best_pos.begin()); offset += dim;
        local_swarm[i].p_best_val = recv_buffer[base + offset++];
        local_swarm[i].current_val = recv_buffer[base + offset++];
    }
}
...
OutputObject pso_mpi(const TestFunction& f, 
    ...
    for (unsigned int i = 0; i < n_points_per_rank; ++i) {
        ...
        for (unsigned int d = 0; d < dim; ++d) {
            ...
            p.p_best_pos[d] = p.position[d]; // Ensure p_best_pos is initialized
        }
    ...
    while (!stop.should_stop(iter, global_best_val)) {
        ...
        for (int s = 0; s < num_sub_swarms; ++s) {
            ...
            for (int i = start; i < end; ++i) {
                if (swarm[i].p_best_val < lbest_val) {
                    lbest_val = swarm[i].p_best_val;
                    lbest_idx = i;
                }
            }
            if (lbest_idx == -1) continue; // skip if no valid lbest
            std::vector<double> lbest_pos = swarm[lbest_idx].p_best_pos;
            for (int i = start; i < end; ++i) {
                Particle& p = swarm[i];
                bool in_bounds = true;
                for (unsigned int d = 0; d < dim; ++d) {
        ...
        for(const auto& p : swarm) {
            if (p.p_best_val < local_min_val) local_min_val = p.p_best_val;
        }
    ...
    for(int i=0; i < (int)swarm.size(); ++i) {
        if(swarm[i].p_best_val < loc_data.val) {
            loc_data.val = swarm[i].p_best_val;
            best_idx_local = i;
        }
    }
    loc_data.rank = rank;
    ...
    if (rank == glob_data.rank) {
        results.x_best = swarm[best_idx_local].p_best_pos;
